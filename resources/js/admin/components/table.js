...
export default class TableView {
    ...
    constructor() {
        ...
        if (this.target) {
            const _data = JSON.parse(this.target.dataset.columns);
            const __data = _data.map((item, index) => {
                if (Device.isMobile() && index > 1 || Device.isTable() && index > 2) {
                    item.hidden = true;
                }
                return item;
            });
            if (Device.isDesktop()) {
                __data.push({
                    name: "Actions",
                    sort: false,
                    width: Device.isTable() ? "auto" : this.target.dataset.actionwidth + "px",
                    formatter: (cell, _) => {
                        let children = [];
            ...
            const grid_js = new Grid({
                ...
                pagination: {
                    limit: window.innerHeight < 800 ? Math.min(10, parseInt(this.target.dataset.paginate)) : this.target.dataset.paginate
                },
                autoWidth: false,
                sort: true,
                search: true,
            ...
            grid_js.render(this.target);
            if (Device.isMobile() || Device.isTable()) {
                grid_js.on('rowClick', (...args) => {
                    const cells = args[1]['_cells'];
                    window.location.href = cells[cells.length - 1].data.show;
                });
            }
